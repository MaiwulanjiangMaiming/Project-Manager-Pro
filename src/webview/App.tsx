...
export default function App() {
  ...
  const filteredProjectsFn = useProjectStore((s) => s.filteredProjects);
  ...
  const filteredProjects = filteredProjectsFn();
  ...
  const handleOpenInNewWindow = useCallback((projectId: string) => {
  ...
  const handleSaveProject = useCallback((project: Project) => {
    sendMessage('saveProject', { project });
  }, [sendMessage]);

  const handleReorderProjects = useCallback((reorderedProjects: Project[], autoSwitchToCustom?: boolean) => {
  ...
  const handleMoveProjectToTag = useCallback((projectId: string, tagId: string) => {
    sendMessage('moveProjectToTag', { projectId, tagId });
  }, [sendMessage]);

  const handleRemoveProjectFromTag = useCallback((projectId: string, tagId: string) => {
  ...
  const handleUpdateTag = useCallback((tag: Tag) => {
  ...
  const handleReorderTags = useCallback((tags: Tag[]) => {
    sendMessage('reorderTags', { tags });
  }, [sendMessage]);

  const handleShowInFolder = useCallback((projectId: string) => {
    sendMessage('showInFolder', { projectId });
  }, [sendMessage]);

  const handleAddToWorkspace = useCallback((projectId: string) => {
  ...
  const handleCreateTask = useCallback((projectId: string, task: Partial<Task>) => {
  ...
  const handleBatchDeleteTasks = useCallback((taskIds: string[]) => {
    sendMessage('batchDeleteTasks', { taskIds });
  }, [sendMessage]);

  const handleBatchUpdateTaskStatus = useCallback((taskIds: string[], status: TaskStatus) => {
    sendMessage('batchUpdateTaskStatus', { taskIds, status });
  }, [sendMessage]);

  const handleCreateMilestone = useCallback((projectId: string, milestone: Partial<Milestone>) => {
    sendMessage('createMilestone', { projectId, ...milestone });
  }, [sendMessage]);
  ...
  const handleCreateChangelog = useCallback((projectId: string, entry: Partial<ChangelogEntry>) => {
    sendMessage('createChangelog', { projectId, ...entry });
  }, [sendMessage]);

  const handleUpdateChangelog = useCallback((entry: ChangelogEntry) => {
    sendMessage('updateChangelog', { entry });
  }, [sendMessage]);

  const handleDeleteChangelog = useCallback((entryId: string) => {
    sendMessage('deleteChangelog', { entryId });
  }, [sendMessage]);

  const handleCreateNote = useCallback((projectId: string, title: string, content: string) => {
  ...
  const handleSaveSnapshot = useCallback((projectId: string, snapshot: Partial<ContextSnapshot>) => {
    sendMessage('saveSnapshot', { projectId, ...snapshot });
  }, [sendMessage]);

  const handleGetLatestSnapshot = useCallback((projectId: string) => {
    sendMessage('getLatestSnapshot', { projectId });
  }, [sendMessage]);
    ...
    <div className="app">
      <div className="app-header">
        <SearchBar
          ...
          onChange={(v) => { setInputSearchValue(v); debouncedSetSearchQuery(v); }}
          sortBy={sortBy}
      ...
      <div className="app-content">
        <div className="global-tasks-bar" style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button
        ...
        {isManageMode && (
          <div className="batch-action-bar">
            <div className="batch-action-left">
              <button className="batch-action-btn" onClick={selectAllProjects}>Select All</button>
              <button className="batch-action-btn" onClick={deselectAllProjects}>Deselect All</button>
              ...
              <div className="settings-panel-body">
                ...
                <div className="settings-item">
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      rpc.send('openExternal', { url: 'https://github.com/MaiwulanjiangMaiming/Project-Manager-Pro' });
                    }}
                  >
                    <svg viewBox="0 0 16 16" fill="currentColor" width="12" height="12" style={{ marginRight: '4px', verticalAlign: 'middle' }}>
                ...
                <div className="settings-item">
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      rpc.send('openExternal', { url: 'https://github.com/MaiwulanjiangMaiming/Project-Manager-Pro#getting-started' });
                    }}
                  >
                    Documentation
                  </a>
                ...
                <div className="settings-item">
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      rpc.send('openExternal', { url: 'https://github.com/MaiwulanjiangMaiming/Project-Manager-Pro/issues/new' });
                    }}
                  >
                    Open GitHub Issue
                  </a>
                ...
                <div className="settings-item">
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      rpc.send('openExternal', { url: 'mailto:mawlan.momin@gmail.com?subject=Project%20Manager%20Pro%20Feedback' });
                    }}
                  >
